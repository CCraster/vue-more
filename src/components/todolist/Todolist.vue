...
import { getGistFiles, formatTodolistData } from '@/utils/util'
...
export default {
  ...
  computed: {
    ...
    selectedTodolistData() {
      if (path([this.selectedTodolistName])(this.todolists)) {
        let todolistData = this.todolists[this.selectedTodolistName].content
    ...
    selectedBlockData() {
      if (
        ...
        return this.selectedTodolistData.todolistContent[this.selectedBlockName]
      }
      return {}
    }
  },
  ...
  methods: {
    ...
    async getTodolist() {
      ...
      this.todolists = getGistFiles(await GistApi.getSingleGist(GIST_TODOLIST))
      loading.close()
    ...
    reconfigTodolist(reconfigTodolistName) {
      let reconfigTodolistContent = this.todolists[reconfigTodolistName].content
      this.addTagDialogConfig.options = {
        title: '更改配置',
        type: 'reconfig',
