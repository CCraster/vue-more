<template>
  ...
  >
    ...
    <div class="todolist-editTime">
      <span>创建时间：{{ timeValueToLocal(singleTodolist.createdTime) }}</span>
      <span>
        最后修改：{{ timeValueToLocal(singleTodolist.lastModifiedTime) }}
      </span>
    </div>
  </div>
</template>
...
import { timeValueToLocal } from '@/utils/util'
...
export default {
  ...
  methods: {
    ...
    handleCopyIconClicked() {
      let text = document.getElementById(`block-${this.singleTodolist.title}`)
        .innerText
