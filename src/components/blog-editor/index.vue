  ...
  <div class="container-blog">
    ...
    <BaseTable :blogs="tableCurrentPageData" :columns="columns">
      ...
      <template slot="label" slot-scope="{ row }">
        <el-tag v-for="(label, index) in row.label" :key="index" size="mini">{{
          label
        }}</el-tag>
      </template>
      ...
      <template slot="opt" slot-scope="{ row }">
        <el-button size="mini" type="primary" @click="editBlogInfo(row)" plain
          >编辑博客</el-button
        >
...
import { getGistFiles, timeValueToLocal } from '@/utils/util'
